...
function buildPrompt(input: ScriptGenerationInput): string {
  ...
  if (input.cta === 'comment_link' && input.rewardLink) {
    body +=
      '\n\n# 댓글 유도 CTA 추가 지시 (필수)\n' +
      `이 게시물의 마지막 슬라이드(CTA)는 "댓글 유도 + 자료 발송" 형태로 작성하세요.\n` +
      `댓글에 특정 키워드(예: "체크", "필요", "도움" 등 — 주제에 맞게)를 적도록 안내하고,\n` +
      `댓글 단 사람에게는 다음 자료를 보내겠다고 자연스럽게 약속하세요:\n` +
      `자료/링크: ${input.rewardLink}\n\n` +
      `예시 톤: "👇 댓글에 'OO' 적어주시면 우리 가족 보험 점검 가이드 PDF 보내드릴게요"\n` +
      `자료의 URL을 슬라이드 main 텍스트에 그대로 노출하지 말고, 댓글 약속만 명시.\n` +
...
export async function generateCaptionAndHashtags(
  ...
  const ctaSection =
    input.cta === 'comment_link' && input.rewardLink
      ? `\n# CTA 모드: 댓글 유도 + 자료 발송\n` +
        `캡션 마지막 단락에 다음을 자연스럽게 안내하세요:\n` +
        `- 댓글에 특정 키워드(주제에 어울리는 1~2자, 예: '체크' '필요' '도움' 등)를 적도록 유도\n` +
        `- 댓글 단 사람에게 다음 자료 발송 약속:\n  ${input.rewardLink}\n` +
        `- 단정적/과장 표현 금지 (광고 심의)\n`
      : `\n# CTA 모드: ${input.cta}\n캡션 마지막 단락에 자연스럽게 ${
          input.cta === 'save' ? '저장' :
